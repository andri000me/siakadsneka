...
<!-- Bootstrap modal -->
<div class="modal fade" id="modal_status_siswa" role="dialog">
    <div class="modal-dialog" >
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Multiple Status Siswa</h3>
            </div>
            <div class="modal-body form" id="save-data-upgrade">
                <form action="#" id="form-upgrade" name="form-upgrade" class="form-horizontal">
                <input name="data_status" type="hidden" id="data-upgrade">
                    ...
                    <div id="data-error-upgrade">
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <h4>Set Status Siswa*</h4>
                                                     <div class="input-group">
                                            <span class="input-group-addon">
                                            <i class="fa fa-check-square"></i>
                                            </span>
                                           <select name="siswa_status2" class="form-control select2me" data-placeholder="Status Siswa" id="upgrade-siswa-status">
                                           <option value=""></option>
            ...
            <div class="modal-footer">
                <button type="button" id="btnUpgrade" onclick="update_status()" class="btn btn-primary"><i class="fa fa-users"></i> Update Status</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#datasiswaaktif').DataTable({
        ...
        buttons: [
           {
               extend: 'print',
               title: 'Data Siswa Tidak Aktif - SMK N 1 Magelang',
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/ajax_list_siswa_tidak_aktif')?>",
            "type": "POST",
             "timeout": 15000,
          ...
          $('td:eq(2),td:eq(4),td:eq(5),td:eq(6),td:eq(7),td:eq(8)', nRow).addClass( "raport-mapel" );
          ...
          //Metronic.initUniform($('input[type="checkbox"]'));
           ...
           "drawCallback": function(oSettings) { // run some code on table redraw
                 ...
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian data: </strong> Ditemukan data dengan jumlah <b>'+ info.recordsDisplay +'</b> siswa <b>Tidak Aktif</b>');

                 var $DataTidakDitemukan = $('<div/>')   // creates a div element
                 .attr('id','wali-data-ofani'+[i])  // adds the id
                 .addClass('alert alert-danger alert-dismissable fade in')   // add a class
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian data: </strong> Maaf sistem tidak menemukan data yang sesuai pada record data</b> siswa <b>Tidak Aktif</b>');
...
$('#status-siswa-cari').prop('disabled', false);
$("#tahun-cari").val('').trigger("change");
$("#kelas-cari").val('').trigger("change");
 $("#status-siswa-cari").val('').trigger("change");
...
$('#tahun-cari').on( 'change', function () {   // for select box
    ...
    table
		...
		if($(this).val() == "")
  		...
  		{
      	$.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/cari_kelas')?>/" + res, {} ,function(obj){
		$('#kelas-cari').html(obj);
		});
...
$('#status-siswa-cari').on( 'change', function () {   // for select box
    table
        .columns( 7 )
        .search( this.value )
        .draw();
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#datasiswaaktif tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#datasiswaaktif tbody tr td div span").attr("checked", checked);
            //});
 ...
 $('.group-checkable').change(function() {
                var set = $("#datasiswaaktif tbody tr td div span input:checkbox");
                var checked = $(this).is(":checked");
                $(set).each(function() {
    ...
    // return  false;
    $.ajax({
        url: '<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/ajax_multiple_delete')?>',
        type: 'post',
        data: 'ids=' + checkValues
    }).done(function(data) {
        $("#respose").html(data);
        window.setTimeout(function() {
                    $(".error-hapus").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
...
$(':reset').live('click', function(){
  ...
  Metronic.blockUI({
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/lihat_data_siswa')?>/" + $('[name="siswa_id"]').val(),
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/cari_kelas_modal')?>/" + res, {} ,function(obj){
            $('#kelas-cari-modal').html(obj);
            $('#kelas-cari-modal').val(data.siswa_kelas);
            $("#kelas-cari-modal").val(data.siswa_kelas).trigger("change");
            $('#kelas-cari-modal').prop('disabled', false);
 ...
 function edit_data(id)
{
 ...
 //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/lihat_data_siswa')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {   
...
{   
    ...
    $('#btnUpgrade').text(' Update Status..').prepend(' <i class="fa fa-users"></i>'); //change button text
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/ajax_multiple_status')?>",
        type: "POST",
        ...
        success: function(data)
        {
            ...
            window.setTimeout(function() {
             ...
             reload_table();
             $('#btnUpgrade').text(' Update Status').prepend(' <i class="fa fa-users"></i>');
...
{   
    ...
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/ajax_update_siswa')?>",
        type: "POST",
        data : formData,
        contentType : false,
        processData : false,
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datasiswa/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<!-- SCRIPTS SCRIPTS DATA SISWA TIDAK AKTIF -->