    ...
    <div class="modal-dialog" >
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Edit Data Peserta Eskul</h3>
            </div>
            <div class="modal-body form" id="edit-data-process">
                <form action="#" id="form" class="form-horizontal">
                    
                    <div class="form-body">
                    <input type="hidden" value="" name="pesertaeskul_id"/> 
                    <input type="hidden" value="" name="pesertaeskul_kelascek" id="kelas-cek"/> 
                            ...
                            <div class="col-md-9">
                               <div class="input-group">
												...
												<i class="fa fa-users "></i>
                                                 ...
                                                 <select name="pesertaeskul_kelas" placeholder="Pilih Kelas" class="form-control select2me" id="data-kelas-aktif2">
                                            <option value=""></option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                           ...
                                           <select name="pesertaeskul_nis" class="form-control select2me" data-placeholder="Pilih Siswa" id="data-siswa-aktif2" disabled="true">
												<option value=""></option>
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Pilih Eskul*</label>
                            <div class="col-md-9">
                               <div class="input-group">
                                            <span class="input-group-addon">
                                            <i class="fa fa-support"></i>
												...
												<select id="data-pilih-eskul2" name="pesertaeskul_dataeskul" class="form-control select2me" data-placeholder="Pilih Eskul">
                                                <option value=""></option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                            ...
                                            <select id="data-status-eskul2" name="pesertaeskul_status" class="form-control select2me" data-placeholder="Pilih Status">
												<option value=""></option>
            ...
            <div class="modal-footer">
                <button type="button" id="btnUpdate" onclick="update()" class="btn btn-primary">Edit Peserta Eskul</button>
...
var table;
//var dt = $('#datapesertaeskul' ).DataTable();
...
jQuery(document).ready(function() {
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#datapesertaeskul').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_list')?>",
            "type": "POST",
             "timeout": 15000,
          ...
          //Metronic.initUniform($('input[type="checkbox"]'));
           ...
           "drawCallback": function(oSettings) { // run some code on table redraw
                 ...
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian : </strong> Ditemukan data <b>peserta eskul</b> tidak wajib sebanyak <b>'+ info.recordsDisplay +'</b> siswa');

                 var $DataTidakDitemukan = $('<div/>')   // creates a div element
                 .attr('id','wali-data-ofani'+[i])  // adds the id
                 .addClass('alert alert-danger alert-dismissable fade in')   // add a class
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian: </strong> Maaf sistem tidak menemukan data yang sesuai pada record data <b>peserta eskul</b>.');
...
$('#data-siswa-aktif').prop('disabled', true);
$("#data-siswa-aktif").val('').trigger("change");
$("#data-kelas-aktif").val('').trigger("change");
$("#data-pilih-eskul").val('').trigger("change");

$("#pencarian-kelas").val('').trigger("change");
$("#pencarian-eskul").val('').trigger("change");

$('#data-kelas-aktif').on( 'change', function () {   // for select box
    	


        var str = $('#data-kelas-aktif').val();
		var res = str.replace("/", "-"); 
		if($(this).val() == "")
  		{
      	 $("#data-siswa-aktif").val('').trigger("change");
  		...
  		{
      	$.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/cari_siswa_aktif')?>/" + str, {} ,function(obj){
		$('#data-siswa-aktif').html(obj);
		});
		
		 $("#data-siswa-aktif").val('').trigger("change");
		 //$('#btnSave').closest('form').find('.select2-offscreen').trigger('change');$("#status-siswa-cari").val('').trigger("change");
		 $('#data-siswa-aktif').prop('disabled', false);
...
$("#data-kelas-aktif2").click(function() {   // for select box
    	


        var str = $('#data-kelas-aktif2').val();
         var str2 = $('#data-siswa-aktif2').val();
		var res = str.replace("/", "-"); 
		if($(this).val() == "")
  		{
      	 $("#data-siswa-aktif2").val('').trigger("change");
      	  $('#data-siswa-aktif2').prop('disabled', true);
		...
		$.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/cari_siswa_aktif')?>/" + str, {} ,function(obj){
		$('#data-siswa-aktif2').html(obj);
		});

		 $("#data-siswa-aktif2").val('').trigger("change");
		 //$('#btnSave').closest('form').find('.select2-offscreen').trigger('change');$("#status-siswa-cari").val('').trigger("change");
		 $('#data-siswa-aktif2').prop('disabled', false);
 ...
 $("#pencarian-reset").click(function() {
  ...
  $('#pencarian-kelas').on( 'change', function () {   // for select box
    table
      ...
      $('#pencarian-eskul').on( 'change', function () {   // for select box
    table
        .columns( 5 )
        .search( this.value )
        .draw();
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#datapesertaeskul tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#datapesertaeskul tbody tr td div span").attr("checked", checked);
            //});
            
 $('.group-checkable').change(function() {
                var set = $("#datapesertaeskul tbody tr td div span input:checkbox");
                var checked = $(this).is(":checked");
                $(set).each(function() {
    ...
    // return  false;
    $.ajax({
        url: '<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_multiple_delete')?>',
        type: 'post',
...
function save()
{
   ...
   $('#btnSave').text(' Tambah Data').prepend(' <i class="fa fa-plus-circle"></i>'); //change button text
    ...
    $('#kelas-cek2').val($('#data-kelas-aktif option:selected').text());
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_tambah')?>",
        type: "POST",
        ...
        success: function(data)
        {
            ...
            {	
				...
				$("#data-kelas-aktif").val('').trigger("change");
				$("#data-pilih-eskul").val('').trigger("change");
				$("#keterangan-waktu").val('').trigger("change");

                 window.setTimeout(function() {
                    $(".notif-tambah-siswa").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
                    }, 1000);
                //get_jumlah();
            ...
            {   
                ...
                var $newDiv = $('<div/>')   // creates a div element
                 .attr('id','wali-data-ofani'+[i])  // adds the id
                 .addClass('notif-tambah-siswa alert alert-danger alert-dismissable fade in')   // add a class
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
 			
            $('[name="pesertaeskul_id"').val(data.pesertaeskul_id);
            $('#data-kelas-aktif2').val(data.siswa_kelas);
            $("#data-kelas-aktif2").val(data.siswa_kelas).trigger("change");
             var str = $('#data-kelas-aktif2').val();
            $.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/cari_siswa_aktif')?>/" + str, {} ,function(obj){
			$('#data-siswa-aktif2').html(obj);
             $("#data-siswa-aktif2").val(data.pesertaeskul_nis).trigger("change");
            $('#data-siswa-aktif2').val(data.pesertaeskul_nis);
            $('#data-siswa-aktif2').prop('disabled', false);
            ...
            $('#data-status-eskul2').val(data.pesertaeskul_status);
            $("#data-status-eskul2").val(data.pesertaeskul_status).trigger("change");

            $('#data-pilih-eskul2').val(data.pesertaeskul_dataeskul);
            $("#data-pilih-eskul2").val(data.pesertaeskul_dataeskul).trigger("change");
            
 		
            //$('#form2')[0].reset();

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Data Peserta Eskul'); // Set title to Bootstrap modal title
...
{   
    ...
    $('#kelas-cek').val($('#data-kelas-aktif2 option:selected').text());
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_update')?>",
        type: "POST",
        data: $('#form').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#btnUpdate').text('Update Peserta Eskul'); //change button text
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datapesertaeskul/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
