<div class="modal fade" id="hapus-modal" tabindex="-1" role="basic" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <h4>Data Tidak Bisa Dikembalikan Setelah Dihapus, Apakah Anda Yakin ?</h4>
                                        </div>
                                        <div class="modal-footer">
                                           
                                            <button type="button" onclick="hapus_nilai()" class="btn bg-red"><i class="fa fa-trash-o"></i> Hapus Nilai</button>
...
jQuery(document).ready(function() {
    ...
    table = $('#datanilaieskulsiswa').DataTable({
        //dom: '<lf<tr>ip>',
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/nonwajib_input_nilaisiswa_edit')?>",
            "type": "POST",
             "data" : {
          ...
          //$('td:eq(6) select',  nRow).addClass('form-control select2me');
          //Metronic.initUniform($('input[type="checkbox"]'));
           ...
           "drawCallback": function(oSettings) { // run some code on table redraw
                        ...
                        $('.tooltips').tooltip();
                         ...
                         $('.class-data-nilai-siswa').select2({
    ...
    $("#nilai-eskul").val('').trigger("change");
    
    $('#nilai-semester').prop('disabled', true);
    $('#nilai-eskul').prop('disabled', true);
    ...
    if($(this).val() == "")
      {
        ...
        $('#nilai-eskul').prop('disabled', true);
         ...
         $('#generate-datanilaieskul').remove();
         $('#file-download-nilai-hasil').remove();
         $('#file-delete-nilai').remove();
      ...
      {
        $.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/cari_semester_nonwajib')?>/" + res, {} ,function(obj){
...
$('#nilai-semester').click(function() {   // for select box
    ...
    if($(this).val() == "")
      ...
      {
       $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/cari_eskul_nonwajib_edit')?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        success: function(result)
        {
        
             $('#nilai-eskul').html(result);
             $("#nilai-eskul").val('').trigger("change");
             $('#nilai-eskul').prop('disabled', false);
...
$('#nilai-eskul').click(function() {   // for select box
  ...
  $('#data-download').click(function() {   // for select box
        
      $("#form-nilai-cari").attr({
                "action" : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/download_formnilai_nonwajib')?>"
                });
    } );

    $('#data-download-nilai').click(function() {   // for select box
        
      $("#form-nilai-cari").attr({
                "action" : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/download_formnilai_nonwajib_hasil')?>"
                });
    } );
...
function hapus_nilai() {
    ...
    var barisdatanilai = $('#datanilaieskulsiswa tbody tr td:nth-child(1) input').map(function()
             {
             return parseInt($(this).val());
            }).get();

    //alert(barisdatanilai);

    $('#datanilaieskul-hapus').val(barisdatanilai);

    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/hapus_nilai_nonwajib_edit')?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
        success: function(data)
        {
               ...
               if (data.status) {
                 ...
                 $('#data-sukses-hapusnilai').append('<div id="sukses-hapus-nilai" class="alert alert-success alert-dismissable fade in" id="wali-data-ofani0"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button> '+ data.sukses_string+'</div>');

                 window.setTimeout(function() {
                    $(".data-nilai").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
                     $("#sukses-form-nilai").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
                    }, 1000);
...
function generate_formnilai() {
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/generate_formnilai_nonwajib')?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
        success: function(data)
        {
                 ...
                 $('#data-generate-formnilai').append('<div id="generate-datanilaieskul" class="note note-info"><div class="row"><div class="col-md-3">Ekstra Kurikuler</div><div class="col-md-3">: <b>'+data.nilaieskul+'</b></div><div class="col-md-3">Kategori Eskul</div><div class="col-md-3">: <b>'+data.nilaijenis+'</b></div></div><div class="row"><div class="col-md-3">Angkatan</div><div class="col-md-3">: <b>'+data.nilaiangkatan+'</b></div><div class="col-md-3">Tahun Ajaran</div><div class="col-md-3">: <b>'+data.nilaitahunajaran+'</b></div></div><div class="row"><div class="col-md-3">Jumlah Peserta</div><div class="col-md-3">: <b>'+data.nilaijumlahsiswa+' siswa</b></div><div class="col-md-3">Semester</div><div class="col-md-3">: <b>'+data.nilaisemester+' ('+ data.nilaisemesterajaran +')</b></div></div><div class="row"></div></div>');
...
function cekformnilai() {
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/validasiformnilai_nonwajib_edit')?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            {   
               ...
               $('#data-download').append('<div class="form-group" id="file-download-nilai"><div class="form-actions noborder"><button type="submit" class="btn green-meadow"><i class="fa fa-download"></i> Download Excel</button></div></div>');
                 ...
                 $('#data-download-nilai').append('<div class="form-group" id="file-download-nilai-hasil"><div class="form-actions noborder"><button type="submit" class="btn green-meadow"><i class="fa fa-download"></i> Download Nilai</button></div></div>');
                ...
                $('#data-sukses-nilai').append('<div id="sukses-form-nilai" class="alert alert-info alert-dismissable fade in" id="wali-data-ofani0"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button> '+ data.suksespesan+'</div>');
                 ...
                 $('#data-delete-nilai').append('<div class="form-group" id="file-delete-nilai"><div class="form-actions noborder"><button type="button" onclick="hapus_nilai_modal()" class="btn bg-red"><i class="fa fa-trash-o"></i> Delete Nilai</button></div></div>');

                generate_formnilai();
                tambah_upload();
                 table.columns(4).search($('#nilai-tahun').val()).draw();
                 window.setTimeout(function() {
                    $(".data-nilai").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
...
{   
    Metronic.blockUI({
     ...
     formData.append("nilai_cari_eskul", $('#nilai-eskul').val());
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/upload_nilai_excel_nonwajib_edit')?>",
        type: "POST",
        data : formData,
        contentType : false,
        processData : false,
        ...
        success: function(data)
        {
            ...
            { 
                ...
                $('#data-error-notif').append($newDiv);
                 ...
                 {
                         $('[name="siswa_nis['+data.data_nis[i]+']"]').val(data.data_nilai[i]).trigger("change");
                 }

                  for (var i = 0; i < data.data_nis.length; i++)
                 {
                         $('[name="siswa_deskripsi['+data.data_nis[i]+']"]').val(data.data_deskripsi[i]);
                 }
                }
...
function updatenilai() {
     ...
     Metronic.blockUI({
                target: '#savenilai-data-process',
                 boxed: true,
                message: 'Proses Mengupdate Nilai...',
                cenrerY: true,
                animate: false
            });

    var barisdatanilai = $('#datanilaieskulsiswa tbody tr td:nth-child(2) input').map(function()
             {
             return parseInt($(this).val());
            }).get();

    $('#datanilaieskul-masuk').val(barisdatanilai);
  $('#btnSave').text(' Memproses Nilai...').prepend(' <i class="fa fa-tasks"></i>'); //change button text
    
     $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilaieskul/edit_multi_nilai_nonwajib')?>",
        type: "POST",
        ...
        success: function(data)
        {
            ...
            window.setTimeout(function() {
             ...
             $('#btnSave').text(' Update Nilai Siswa').prepend(' <i class="fa fa-tasks"></i>');
              //$('#generate-datanilai').remove();
              //generate_formnilai();
              //table.columns(4).search($('#nilai-kelas').val()).draw();
              cekformnilaiUpdate();
             DataPesanNilaiSuksesEdit();
...
<!-- SCRIPT DATA NILAI ESKUL EDIT (NON WAJIB)-->