...
jQuery(document).ready(function() {
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#datasiswaaktif').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('siswa/datasiswa/ajax_list_siswa_tidak_aktif')?>",
            "type": "POST",
             "timeout": 15000,
...
$('#tahun-cari').on( 'change', function () {   // for select box
    ...
    table
		...
		if($(this).val() == "")
  		...
  		{
      	$.post("<?php echo site_url('siswa/datasiswa/cari_kelas')?>/" + res, {} ,function(obj){
		$('#kelas-cari').html(obj);
		});
...
$('#tahun-cari-modal').click(function() {   // for select box
    ...
    if($(this).val() == "")
      ...
      {
        $.post("<?php echo site_url('siswa/datasiswa/cari_kelas_modal')?>/" + res, {} ,function(obj){
...
function lihat_data_siswa(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('siswa/datasiswa/lihat_data_siswa')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
<!-- SCRIPTS SCRIPTS DATA SISWA TIDAK AKTIF : SISWA -->