        ...
        <div class="modal-content">
            ...
            <div class="modal-body form" id="edit-data-process">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                    <input type="hidden" value="" name="mapel_id"/>
            ...
            <div class="modal-footer">
                <button type="button" id="btnUpdate" onclick="update()" class="btn btn-primary">Edit Mapel</button>
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#datamapel').DataTable({
        ...
        buttons: [
           {
               extend: 'print',
               title: 'Data Mapel - SMK N 1 Magelang',
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datamapel/ajax_list') ?>",
            "type": "POST",
          ...
          $('td:eq(2),td:eq(3),td:eq(4)', nRow).addClass( "raport-mapel" );
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#datamapel tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#datamapel tbody tr td div span").attr("checked", checked);
            //});

 $('.group-checkable').change(function() {
                var set = $("#datamapel tbody tr td div span input:checkbox");
                var checked = $(this).is(":checked");
                $(set).each(function() {
...
function save()
{
   ...
   $('#btnSave').text(' Tambah Data...').prepend(' <i class="fa fa-plus-circle"></i>'); //change button text
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datamapel/ajax_add') ?>",
        type: "POST",
        ...
        success: function(data)
        {
            ...
            else
            {
                ...
                $('#data-error-mapel').append($newDiv);
            ...
            $('#btnSave').text(' Tambah Mapel').prepend(' <i class="fa fa-plus-circle"></i>'); //change button text
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
            ...
            DataPesanError4();
            $('#btnSave').text(' Tambah Mapel'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable
            //location.reload();
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datamapel/ajax_edit') ?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="mapel_sort"]').val(data.mapel_sort);
            $('[name="mapel_status"]').val(data.mapel_status);
            $('#form2')[0].reset();
...
function update()
{
    ...
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datamapel/ajax_update') ?>",
        type: "POST",
        data: $('#form').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#btnUpdate').text('Update Mapel'); //change button text
