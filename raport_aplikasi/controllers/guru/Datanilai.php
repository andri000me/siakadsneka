<?php
class Datanilai extends Gururaport_Controller {
    ...
    public function input_nilai() {
      ...
      $this->data['data_angkatan_tidakaktif2'] = $this->siswa_m->get_data_angkatan_guru2();
    	...
    	$this->data['subview'] = 'guru/datanilai/InputNilai';
     ...
     public function edit_nilai() {
       ...
       $this->data['infonilai'] = '<div id="sukses-form-nilai" class="alert alert-info alert-dismissable fade in"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button> <i class="fa fa-info-circle"></i> <strong>Info:</strong> Maaf <b>Bpk/Ibu '.$this->get_namaguru().'</b>, saat ini anda tidak memiliki tugas untuk mengedit data<b> NILAI SISWA </b>.</div>';
        ...
        $this->data['data_angkatan_tidakaktif2'] = $this->siswa_m->get_data_angkatan_guru2();
        //Load Data View Data Nilai Input UH
        $this->data['subview'] = 'guru/datanilai/EditNilai';
        $this->load->view('guru/admindesain', $this->data);
        ...
        $datakelas = $this->get_datakelas($this->input->post('nilai_cari_kelas'));
          ...
          $objPHPExcel->setActiveSheetIndex(0)
            ...
            if ($this->status_kelas($this->input->post('nilai_cari_angkatan')) == 'aktif') {
             ...
             if ($this->cekdatamasuk($this->input->post('nilai_cari_kelas'),$this->input->post('nilai_cari_angkatan'),$this->input->post('nilai_cari_mapel'),$this->input->post('nilai_cari_semester')) == 1) {
               if(!empty($data)){
            ...
            $objPHPExcel->getActiveSheet()->mergeCells('A13:F13');
              ...
              $objPHPExcel->setActiveSheetIndex(0)
                                        ->setCellValue('A13', 'MAAF ANDA TIDAK MEMILIKI HAK UNTUK MENGAKSES DATA NILAI KELAS INI');
   ...
   if(!empty($datamapel)){
    ...
    private function get_tahunajaran($id) {
        $query = $this->db->query('SELECT haknilai_tahunajaran FROM raport_haknilai LEFT JOIN  raport_kelas ON raport_kelas.kelas_code = raport_haknilai.haknilai_kelas WHERE kelas_code="'.$this->db->escape_str($id).'" AND haknilai_kodeguru = "'.$this->session->userdata('user_login').'" AND haknilai_status="1"');
