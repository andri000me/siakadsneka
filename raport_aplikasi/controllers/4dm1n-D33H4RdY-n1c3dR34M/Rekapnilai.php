<?php
class Rekapnilai extends Adminraport_Controller {
    ...
    public function penilaianakhir() {
       ...
       $this->data['data_angkatan_tidakaktif2'] = $this->siswa_m->get_data_angkatan2();
    	...
    	$this->data['subview'] = 'admin/rekapnilai/penilaianakhir';
    ...
    public function penilaianberproses() {
      ...
      //Load Data View Penilaian Akhir
      $this->data['subview'] = 'admin/rekapnilai/penilaianberproses';
      $this->load->view('admin/admindesain', $this->data);
      
    }

    public function cekdb($kelas, $tahunajaran, $semester, $mapel) {

      $tahun = str_replace('-','/', $tahunajaran);
    ...
    LEFT JOIN raport_guru ON raport_guru.guru_kode = raport_nilai.nilai_kodeguru WHERE nilai_tahun = "'.$tahun.'" AND nilai_semester = "'.$semester.'"
   ...
   public function ajax_list_rekap()
    ...
    public function halobro() {
      ...
      echo $this->rumusrapormapel_m->EVALNILAIUH('41','3', '2016/2017','71');
      echo '<br>';

      echo  $this->rumusrapormapel_m->jumlah_tabelTG('41','3', '2016/2017','71');

      $this->rumusrapormapel_m->EVALNILAIUH('41','3', '2016/2017','71');

      dump('if ($rekap->RATA_PO == NULL) {
              $RATA_PO ="<i class=fa fa-minus></i>";
            } else {
              $RATA_PO = "<b>".$rekap->RATA_PO."</b>";
            }');
   ...
   if(!empty($datamapel)){
    ...
    public function halofani() {
      echo $this->nomoralfa(0);
      echo '<br>';
      echo $this->nomoralfa(2);
      echo '<br>';
      echo $this->nomoralfa(100);
    }
    ...
    private function status_kelas($status)
    {
     ...
     public function cari_mapel() {
      ...
      if ($this->input->post('rekap_semester') == 5 || $this->input->post('rekap_semester') == 6 ) {
           $data   = $this->nilai_m->get_data_mapel2_rekap($semester5dan6, $this->input->post('rekap_kelas'));
           $tahunajaran = '<optgroup label="Data Mapel TH :'.$semester5dan6.'">';
      } elseif ($this->input->post('rekap_semester') == 3 || $this->input->post('rekap_semester') == 4) {
          $data   = $this->nilai_m->get_data_mapel2_rekap($semester3dan4, $this->input->post('rekap_kelas'));
          $tahunajaran = '<optgroup label="Data Mapel TH :'.$semester3dan4.'">';
      } elseif ($this->input->post('rekap_semester') == 1 || $this->input->post('rekap_semester') == 2) {
          $data   = $this->nilai_m->get_data_mapel2_rekap($semester1dan2, $this->input->post('rekap_kelas'));
    ...
    private function validasi_cari() {
        ...
        if ($this->input->post('rekap_tahun') == '') {
            ...
            $data['error_string'][] = '<i class="fa fa-warning"></i> <strong>Warning:</strong> Anda belum memilih data <b>tahun angkatan</b>.';
            $data['status'] = FALSE;
        }
        ...
        if ($this->input->post('rekap_kelas') == '') {
            ...
            $data['inputerror'][] = 'data_masuk';
            $data['error_string'][] = '<i class="fa fa-warning"></i> <strong>Warning:</strong> Anda belum memilih data <b>kelas</b>.';
            $data['status'] = FALSE;
        }
        ...
        if ($this->input->post('rekap_semester') == '') {
            ...
            $data['inputerror'][] = 'data_masuk';
            $data['error_string'][] = '<i class="fa fa-warning"></i> <strong>Warning:</strong> Anda belum memilih data <b>semester</b>.';
            $data['status'] = FALSE;
        }
        ...
        if ($this->input->post('rekap_mapel') == '') {
            ...
            $data['inputerror'][] = 'data_masuk';
            $data['error_string'][] = '<i class="fa fa-warning"></i> <strong>Warning:</strong> Anda belum memilih data <b>mata pelajaran</b>.';
            $data['status'] = FALSE;
        }
        ...
        if ($this->cekdatamapel($this->input->post('rekap_mapel')) < 1 && trim($this->input->post('rekap_mapel') !== '') && trim($this->input->post('rekap_mapel') !== NULL)) {
            $data['inputerror'][] = 'data_masuk';
            $data['error_string'][] = '<i class="fa fa-warning"></i> <strong>Warning:</strong> Data <b>mata pelajaran</b>, yang anda masukkan <b>tidak valid</b>.';
            $data['status'] = FALSE;
        }
