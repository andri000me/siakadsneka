...
jQuery(document).ready(function() {
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#dataabsensi').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('guru/dataabsensi/ajax_list')?>",
            "type": "POST",
             "timeout": 15000,
...
$("#keterangan-absensi").val('').trigger("change");
...
$('#data-kelas-aktif').on( 'change', function () {   // for select box
		...
		if($(this).val() == "")
  		...
  		{
      	$.post("<?php echo site_url('guru/dataabsensi/cari_siswa_aktif')?>/" + str, {} ,function(obj){
...
$("#data-kelas-aktif2").click(function() {   // for select box
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#dataabsensi tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#dataabsensi tbody tr td div span").attr("checked", checked);
            //});
 ...
 $('.group-checkable').change(function() {
    ...
    $("#del_all_backup").on('click', function(e) {
    e.preventDefault();
    ...
    // return  false;
    $.ajax({
        url: '<?php echo site_url('guru/dataabsensi/ajax_multiple_delete')?>',
        type: 'post',
...
function hapus_multiple() {
    if(confirm('Apakah anda yakin akan menghapus data ini ? Data yang sudah terhapus tidak dapat dikembalikan.'))
    {
    var checkValues = $('.checkbox1:checked').map(function()
    {
        return $(this).val();
     ...
     $.ajax({
            ...
            {   
                ...
                {  
                 ...
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button> '+ data.sukses_string[i]);
                
                $('#respose').append($newDiv);
               }
                
                window.setTimeout(function() {
                    $(".data-hapus").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
                    });
                    $(".tutup-mapel-hr").fadeTo(1500, 0).slideUp(500, function(){
                        $(this).remove(); 
            ...
            {   
                ...
                var $newDiv = $('<div/>')   // creates a div element
                 .attr('id','wali-data-ofani'+[i])  // adds the id
                 .addClass('alert data-hapus alert-danger alert-dismissable fade in')   // add a class
...
function get_jumlah() {
    ...
    $.ajax({
        url : "<?php echo site_url('guru/count/hitung')?>",
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
            ...
            if (data.jumlah_absensi == '0') {
             ...
             if (data.jumlah_absensi_all == '0') {
                 $("#jumlahabsensi_all").attr({
                    "class" : "badge badge-danger",
                 }).text('EMPTY');
            } else {
                 $("#jumlahabsensi_all").attr({
...
function save()
{
   ...
   $('#btnSave').text(' Tambah Data').prepend(' <i class="fa fa-plus-circle"></i>'); //change button text
    ...
    $.ajax({
        url : "<?php echo site_url('guru/dataabsensi/ajax_tambah')?>",
        type: "POST",
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('guru/dataabsensi/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
 			
            $('[name="absensi_id"').val(data.absensi_id);
            $('#data-kelas-aktif2').val(data.siswa_kelas);
            $("#data-kelas-aktif2").val(data.siswa_kelas).trigger("change");
             var str = $('#data-kelas-aktif2').val();
            $.post("<?php echo site_url('guru/dataabsensi/cari_siswa_aktif')?>/" + str, {} ,function(obj){
			$('#data-siswa-aktif2').html(obj);
             $("#data-siswa-aktif2").val(data.absensi_nis).trigger("change");
            $('#data-siswa-aktif2').val(data.absensi_nis);
            $('#data-siswa-aktif2').prop('disabled', false);
		
		});

           
            $('#keterangan-waktu2').val(data.absensi_waktu);
  
            $("#keterangan-absensi2").val(data.absensi_keterangan).trigger("change");
            
 		
            //$('#form2')[0].reset();

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Data Absensi'); // Set title to Bootstrap modal title
...
{   
    ...
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('guru/dataabsensi/ajax_update')?>",
        type: "POST",
        data: $('#form').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#btnUpdate').text('Update Absensi'); //change button text
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('guru/dataabsensi/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
