        ...
        <div class="modal-content">
            ...
            <div class="modal-body form" id="edit-data-process">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                    <input type="hidden" value="" name="prestasi_id"/> 
                    <input type="hidden" value="" name="prestasi_kelascek" id="kelas-cek"/> 
                        ...
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            <label class="control-label col-md-3">Nama Prestasi*</label>
                            <div class="col-md-9">
                                <div class="input-group">
                                            <span class="input-group-addon">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                               <div>
                                            <textarea name="prestasi_deskripsi" class="form-control" rows="2"></textarea>
                                        </div>
                                <span class="help-block"></span>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button type="button" id="btnUpdate" onclick="update()" class="btn btn-primary">Update Prestasi</button>
...
jQuery(document).ready(function() {
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#dataprestasi').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/ajax_list')?>",
            "type": "POST",
             "timeout": 15000,
          ...
          $('td:eq(2),td:eq(4),td:eq(6),td:eq(7),td:eq(8)', nRow).addClass( "raport-mapel" );
          //Metronic.initUniform($('input[type="checkbox"]'));
           ...
           "drawCallback": function(oSettings) { // run some code on table redraw
                 ...
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian : </strong> Ditemukan data <b>Prestasi Siswa</b> sebanyak <b>'+ info.recordsDisplay +'</b> data');

                 var $DataTidakDitemukan = $('<div/>')   // creates a div element
                 .attr('id','wali-data-ofani'+[i])  // adds the id
                 .addClass('alert alert-danger alert-dismissable fade in')   // add a class
                 .html(' <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><i class="fa fa-users"></i> <strong>Hasil pencarian: </strong> Maaf sistem tidak menemukan data yang sesuai pada record data <b>Prestasi Siswa</b>.');
...
$("#pencarian-bidang-prestasi").val('').trigger("change");
$("#pencarian-tingkat-prestasi").val('').trigger("change");
$("#pencarian-peringkat-prestasi").val('').trigger("change");
...
$('#data-kelas-aktif').on( 'change', function () {   // for select box
		...
		if($(this).val() == "")
  		...
  		{
      	$.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/cari_siswa_aktif')?>/" + str, {} ,function(obj){
...
$("#pencarian-reset").click(function() {
  ...
  $('#pencarian-bidang-prestasi').on( 'change', function () {   // for select box
    table
        .columns( 5 )
        .search( this.value )
        .draw();
      ...
      $('#pencarian-tingkat-prestasi').on( 'change', function () {   // for select box
    table
        .columns( 6 )
        .search( this.value )
        .draw();
      ...
      $('#pencarian-peringkat-prestasi').on( 'change', function () {   // for select box
    table
        .columns( 7 )
        .search( this.value )
        .draw();
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#dataprestasi tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#dataprestasi tbody tr td div span").attr("checked", checked);
            //});
            
 $('.group-checkable').change(function() {
                var set = $("#dataprestasi tbody tr td div span input:checkbox");
                var checked = $(this).is(":checked");
                $(set).each(function() {
    ...
    // return  false;
    $.ajax({
        url: '<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/ajax_multiple_delete')?>',
        type: 'post',
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
 			
            $('[name="prestasi_id"').val(data.prestasi_id);
            $('#data-kelas-aktif2').val(data.siswa_kelas);
            $("#data-kelas-aktif2").val(data.siswa_kelas).trigger("change");
             var str = $('#data-kelas-aktif2').val();
            $.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/cari_siswa_aktif')?>/" + str, {} ,function(obj){
			      $('#data-siswa-aktif2').html(obj);
            $("#data-siswa-aktif2").val(data.prestasi_nis).trigger("change");
            $('#data-siswa-aktif2').val(data.prestasi_nis);
            $('#data-siswa-aktif2').prop('disabled', false);
            ...
            $('#data-bidang-prestasi').val(data.prestasi_bidang);
            $("#data-bidang-prestasi").val(data.prestasi_bidang).trigger("change");
            $('#data-tingkat-prestasi').val(data.prestasi_tingkat);
            $("#data-tingkat-prestasi").val(data.prestasi_tingkat).trigger("change");
            $('#data-peringkat-prestasi').val(data.prestasi_peringkat);
            $("#data-peringkat-prestasi").val(data.prestasi_peringkat).trigger("change");
            $('[name="prestasi_nama"').val(data.prestasi_nama);
            $('[name="prestasi_deskripsi"').val(data.prestasi_deskripsi);
...
{   
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/ajax_update')?>",
        type: "POST",
        data: $('#form').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#btnUpdate').text('Update Prestasi'); //change button text
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/dataprestasi/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<!-- SCRIPTS DATA PRESTASI LIHAT DATA-->