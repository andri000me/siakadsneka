<?php
class Cetakraport extends Siswaraport_Controller {
    ...
    public function cover_html($nis=NULL) {
        ...
        if ($this->uri->segment(4) !== $this->session->userdata('user_login')) {
            ;
            show_404(uri_string());
        }
    ...
    private function get_tahun($nis)
    {
        ...
        $query = $this->db->query('SELECT kelas_tahun, siswa_nis FROM raport_siswa LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_siswa.siswa_kelas WHERE siswa_nis = "'.$nis.'"');
        //$query = $this->db->get();
    ...
    private function raport_nilaisikap($semester, $tahun, $nis) {
        ...
        $tmp  = '';
         ...
         if(!empty($data1)){
          ...
          <td style="width:198.75pt;border:solid windowtext 1.5pt;
          border-top:none;padding:0in 5.4pt 0in 5.4pt;height:391.5pt" valign="top" width="265">
          <p class="MsoNormal" style="margin-bottom:0in;margin-bottom:.0001pt;
          text-align:center;line-height:normal" align="center"><b><span style="font-size:12.0pt">'.$datanilai.'</span></b></p>
          </td>
          ...
          normal"><span style="font-size:12.0pt">'.$sikapdeskripsi.'</span></p>
          </td>
         </tr>';
        }
        } else {
  ...
  normal"><span style="font-size:12.0pt">-</span></p>
    ...
    <td style="width:29.2pt;border:solid windowtext 1.5pt;border-top:
    ...
    private function cekC2($semester, $tahun, $nis)
    {
        // Do NOT validate if email already exists
        // UNLESS it's the email for the current user
        $query = $this->db->query("select kompetensi_kelompok FROM raport_nilai LEFT JOIN raport_siswa ON raport_siswa.siswa_nis = raport_nilai.nilai_nis LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_nilai.nilai_kelas LEFT JOIN raport_mapel ON raport_mapel.mapel_id = raport_nilai.nilai_mapel LEFT JOIN (SELECT kompetensi_nama, kompetensi_pengetahuan, kompetensi_keterampilan, kompetensi_sikap, kompetensi_kelompok, kompetensi_mapel from raport_kompetensi WHERE kompetensi_semesterfilter ='".$semester."') as raport_kompetensi2 ON raport_kompetensi2.kompetensi_mapel = raport_nilai.nilai_mapel LEFT JOIN (SELECT haknilai_mapel, haknilai_kkm, haknilai_kkm2 FROM raport_haknilai WHERE haknilai_tahunajaran='".$tahun."') as raport_haknilai2 ON raport_haknilai2.haknilai_mapel = raport_nilai.nilai_mapel WHERE siswa_nis = '".$nis."' AND nilai_semester = '".$semester."' AND kompetensi_kelompok ='C2' group by siswa_nis, nilai_mapel order by mapel_sort");
        //$query = $this->db->get();
     ...
     private function cekM($semester, $tahun, $nis)
    {
        // Do NOT validate if email already exists
        // UNLESS it's the email for the current user
        $query = $this->db->query("select kompetensi_kelompok FROM raport_nilai LEFT JOIN raport_siswa ON raport_siswa.siswa_nis = raport_nilai.nilai_nis LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_nilai.nilai_kelas LEFT JOIN raport_mapel ON raport_mapel.mapel_id = raport_nilai.nilai_mapel LEFT JOIN (SELECT kompetensi_nama, kompetensi_pengetahuan, kompetensi_keterampilan, kompetensi_sikap, kompetensi_kelompok, kompetensi_mapel from raport_kompetensi WHERE kompetensi_semesterfilter ='".$semester."') as raport_kompetensi2 ON raport_kompetensi2.kompetensi_mapel = raport_nilai.nilai_mapel LEFT JOIN (SELECT haknilai_mapel, haknilai_kkm, haknilai_kkm2 FROM raport_haknilai WHERE haknilai_tahunajaran='".$tahun."') as raport_haknilai2 ON raport_haknilai2.haknilai_mapel = raport_nilai.nilai_mapel WHERE siswa_nis = '".$nis."' AND nilai_semester = '".$semester."' AND kompetensi_kelompok ='M' group by siswa_nis, nilai_mapel order by mapel_sort");
        //$query = $this->db->get();
