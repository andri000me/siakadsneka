...
jQuery(document).ready(function() {
    ...
    table = $('#datanilaisiswa').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/input_nilaisiswa_edit') ?>",
            "type": "POST",
             "data" : {
                    ...
                    return $("#nilai-mapel").val();
                        ...
                        nilai_cari_aspek ( d ) {
                    return $("#nilai-aspek-edit").val();
                        },
                        nilai_cari_data ( d ) {
                    return $("#nilai-data").val();
                        },
             },
             "timeout": 15000,
            ...
            "language": {
                "search": "Cari Data: ",
                "emptyTable": "Form Data Nilai Siswa Tidak Tersedia (Silahkan mengisi form diatas, sebelum mengedit data nilai)",
               "info": "Menampilkan _START_ sampai _END_ dari _TOTAL_ entri ",
                "infoEmpty": "No entries found",
                "infoFiltered": "(filtered1 from _MAX_ total entries)",
                "search": "Search:",
                "processing":   "Sedang memproses...",
                "zeroRecords": "Form Data Nilai Siswa Tidak Tersedia (Silahkan mengisi form diatas, sebelum mengedit data nilai)",
  ...
  $("#nilai-aspek-edit").val('').trigger("change");
  $("#nilai-data").val('').trigger("change");

  $('#nilai-kelas').prop('disabled', true);
  $('#nilai-semester').prop('disabled', true);
  $('#nilai-mapel').prop('disabled', true);
  $('#nilai-aspek-edit').prop('disabled', true);
  $('#nilai-data').prop('disabled', true);
  ...
  $('#nilai-tahun').click(function() {   // for select box
    ...
    $('#nilai-data').prop('disabled', true);
      ...
      {
        $.post("<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/cari_kelas_modal') ?>/" + res, {} ,function(obj){
...
$('#nilai-semester').click(function() {   // for select box
    ...
    $('#nilai-data').prop('disabled', true);
      ...
      {
       $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/cari_mapel') ?>",
        type: "POST",
...
$('#nilai-mapel').click(function() {   // for select box
    ...
    $('#nilai-data').prop('disabled', true);
      ...
      else
      {
         ...
         $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/cari_aspek') ?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        success: function(result)
        {

             $('#nilai-aspek-edit').html(result);

             $('#nilai-aspek-edit').prop('disabled', false);
             $("#nilai-aspek-edit").val('').trigger("change");
             $("#nilai-data").val('').trigger("change");
             $('#nilai-data').prop('disabled', true);
             $('#file-download-nilai').remove();
...
$('#nilai-aspek-edit').click(function() {   // for select box
    ...
    $('#nilai-data').prop('disabled', true);
      ...
      else
      {
           ...
           $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/cari_data_nilai') ?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        success: function(result)
        {

             $('#nilai-data').html(result);

             $("#nilai-data").val('').trigger("change");
             $('#nilai-data').prop('disabled', false);
             $('#file-download-nilai').remove();
             $('#file-download-nilai-hasil').remove();
             $('#file-delete-nilai').remove();
             $('#form-upload-excel').remove();
...
$('#nilai-data').click(function() {   // for select box
...
$('#data-download').click(function() {   // for select box
  ...
  $("#form-nilai-cari").attr({
            "action" : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/download_formnilai_edit') ?>"
            });
} );
...
$('#data-download-nilai').click(function() {   // for select box
  ...
  $("#form-nilai-cari").attr({
            "action" : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/download_formnilai_edit_hasil') ?>"
            });
} );
...
function generate_formnilai() {
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/generate_formnilai_edit') ?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
...
function hapus_nilai() {
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/hapus_nilai') ?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
        success: function(data)
...
function cekformnilai() {
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/validasiformnilai_edit') ?>",
        type: "POST",
        data: $('#form-nilai-cari').serialize(),
        dataType: "JSON",
        success: function(data)
        {
            ...
            if(data.status) //if success close modal and reload ajax table
            {
                ...
                if ($('#nilai-aspek-edit').val() == 'RAPORT_P' || $('#nilai-aspek-edit').val() == 'RAPORT_K' || $('#nilai-aspek-edit').val() == 'RAPORT_S') {
...
function upload_nilai()
{
    Metronic.blockUI({
     ...
     formData.append("nilai_cari_aspek", $('#nilai-aspek-edit').val());
     formData.append("nilai_cari_data", $('#nilai-data').val());
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/upload_nilai_excel_edit') ?>",
        type: "POST",
        data : formData,
        contentType : false,
        processData : false,
...
function updatenilai() {
  ...
  $('#btnSave').text(' Memproses Nilai...').prepend(' <i class="fa fa-tasks"></i>'); //change button text
     ...
     $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datanilai/edit_multi_nilai') ?>",
        type: "POST",
...
<!-- SCRIPT DATA NILAI EDIT NILAI MAPEL -->