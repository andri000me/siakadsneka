...
class Nilaieskul_M extends MY_Model {

	protected $table = 'raport_nilaieskul';
	protected $column = array('siswa_nis','siswa_nama','kelas_kk','kelas_nama','siswa_absen', 'nilaieskul_data', 'kelas_tahun','siswa_status','siswa_kelas'); //set column field database for order and search
	protected $column_nonwajib = array('siswa_nis','siswa_nama','kelas_kk','kelas_nama','nilaieskul_data', 'kelas_tahun','siswa_status','siswa_kelas');
	protected $order = array('siswa_absen' => 'asc'); // default order 
	protected $order_nonwajib = array('pesertaeskul_id' => 'asc'); // default order 
	protected $primary_id = 'nilaieskul_id';
	protected $_primary_key = 'nilaieskul_id';
	protected $_table_name = 'raport_nilaieskul';
	 ...
	 public function _ambildatasiswa($kelas,$status)
	{
	 ...
	 public function _ambildatanilaisiswa($kelas, $eskul, $semester, $status)
	{
        ...
        $this->db->where('nilaieskul_dataeskul', $eskul);
        $this->db->where('nilaieskul_semester', $semester);
	 ...
	 public function _ambildatasiswa_nonwajib($eskul, $semester, $status)
	{
	...
	public function _ambildatasiswa_nonwajib_edit($pesertatahunajaran, $pesertadataeskul, $nilaitahunajaran, $nilaidataeskul, $nilaisemester, $status)
	{
        ...
        $this->db->where('nilaieskul_tahunajaran', $nilaitahunajaran);
        $this->db->where('nilaieskul_dataeskul', $nilaidataeskul);
        $this->db->where('nilaieskul_semester', $nilaisemester);
        //$this->db->where('kelas_tahun', $tahun);
        //$this->db->where('nilaieskul_tahunajaran', $tahun);
	...
	public function get_datatables_datanilaisiswa($kelas, $eskul, $semester, $status)
	{
		$this->_ambildatanilaisiswa($kelas, $eskul, $semester, $status);
	...
	public function count_filtered_datanilaisiswa($kelas, $eskul, $semester, $status)
	{
		$this->_ambildatanilaisiswa($kelas, $eskul, $semester, $status);
		$query = $this->db->get();
	...
	public function count_all_datanilaisiswa($kelas, $eskul, $semester, $status)
	{
		 $this->db->select('nilaieskul_id,siswa_nis,siswa_nama,kelas_nama, nilaieskul_data, nilaieskul_deskripsi, siswa_absen,kelas_tahun,siswa_status,kelas_kk, siswa_kelas');
        $this->db->from('raport_nilaieskul');
        $this->db->join('(SELECT siswa_nis,siswa_nama, siswa_absen, kelas_nama, kelas_tahun, kelas_kk, siswa_kelas, siswa_status FROM raport_siswa LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_siswa.siswa_kelas) as data_siswa', 'data_siswa.siswa_nis = raport_nilaieskul.nilaieskul_nis', 'left');
    ...
    public function get_data_eskuledit_guru($kelas, $semester, $tahun) {
        $query = $this->db->query('SELECT DISTINCT eskul_id, eskul_nama FROM raport_nilaieskul LEFT JOIN (SELECT siswa_nis,siswa_nama,siswa_kelas, kelas_tahun FROM raport_siswa LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_siswa.siswa_kelas) as data_siswa ON data_siswa.siswa_nis = raport_nilaieskul.nilaieskul_nis LEFT JOIN raport_eskul ON raport_eskul.eskul_id = raport_nilaieskul.nilaieskul_dataeskul LEFT JOIN raport_hakeskul ON raport_hakeskul.hakeskul_ideskul = raport_nilaieskul.nilaieskul_dataeskul WHERE eskul_kategori=1 AND eskul_status=1 AND siswa_kelas="'.$this->db->escape_str($kelas).'" AND nilaieskul_semester="'.$this->db->escape_str($semester).'" AND eskul_tahunajaran="'.$this->db->escape_str($tahun).'" AND hakeskul_kodeguru="'.$this->session->userdata('user_login').'" AND hakeskul_status="1"');
        
        if ($query->num_rows() > 0) return $query->result();              
	}
	
	public function get_data_eskuledit_guru_nonwajib_v2($kelas, $semester, $tahun) {
        $query = $this->db->query('SELECT DISTINCT eskul_id, eskul_nama FROM raport_nilaieskul LEFT JOIN (SELECT siswa_nis,siswa_nama,siswa_kelas, kelas_tahun FROM raport_siswa LEFT JOIN raport_kelas ON raport_kelas.kelas_code = raport_siswa.siswa_kelas) as data_siswa ON data_siswa.siswa_nis = raport_nilaieskul.nilaieskul_nis LEFT JOIN raport_eskul ON raport_eskul.eskul_id = raport_nilaieskul.nilaieskul_dataeskul LEFT JOIN raport_hakeskul ON raport_hakeskul.hakeskul_ideskul = raport_nilaieskul.nilaieskul_dataeskul WHERE eskul_kategori=2 AND eskul_status=1 AND siswa_kelas="'.$this->db->escape_str($kelas).'" AND nilaieskul_semester="'.$this->db->escape_str($semester).'" AND eskul_tahunajaran="'.$this->db->escape_str($tahun).'" AND hakeskul_kodeguru="'.$this->session->userdata('user_login').'" AND hakeskul_status="1"');
     ...
     public function get_data_mapel3($kelas, $tahun) {
        $query = $this->db->query('SELECT mapel_id, mapel_nama FROM raport_haknilai LEFT JOIN raport_mapel ON raport_mapel.mapel_id = raport_nilaieskul.nilaieskul_mapel WHERE nilaieskul_kelas="'.$this->db->escape_str($kelas).'" AND nilaieskul_tahunajaran="'.$this->db->escape_str($tahun).'" ');
        
        if ($query->num_rows() > 0) return $query->result();              
    }

     public function get_data_nilaieskul($kelas, $mapel, $semester, $jenis) {
        $query = $this->db->query('SELECT DISTINCT(nilaieskul_jenis) FROM raport_nilaieskul WHERE nilaieskul_kelas="'.$this->db->escape_str($kelas).'" AND nilaieskul_mapel="'.$this->db->escape_str($mapel).'" AND nilaieskul_semester="'.$this->db->escape_str($semester).'" AND nilaieskul_jenis LIKE "'.$jenis.'%" ');
    ...
    public function hapus_nilaieskul($kelas, $mapel, $semester, $jenis)
	{
		
		$this->db->where('nilaieskul_kelas', $kelas);
		$this->db->where('nilaieskul_mapel', $mapel);
		$this->db->where('nilaieskul_semester', $semester);
		$this->db->where('nilaieskul_jenis', $jenis);
