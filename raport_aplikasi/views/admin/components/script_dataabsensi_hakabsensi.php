    ...
    <div class="modal-dialog" >
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Edit Data Hak Absensi</h3>
            </div>
            <div class="modal-body form" id="edit-data-process">
                <form action="#" id="form" class="form-horizontal">
                    
                    <div class="form-body">
                    <input type="hidden" value="" name="hakabsensi_id"/>
                    <input type="hidden" value="" name="hakabsensi_kelascek" id="kelas-cek"/> 
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                           ...
                                           <?php $attributes = array('name' => 'hakabsensi_kodeguru', 'class' => 'form-control select2me', 'data-placeholder' => 'Pilih Guru', 'id' => 'data-select2-raport3');
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                            ...
                                            <?php $attributes = array('name' => 'hakabsensi_kelas', 'class' => 'form-control select2me', 'data-placeholder' => 'Pilih Kelas','id' => 'data-select2-raport4');
...
var table;
//var dt = $('#datahakabsensi' ).DataTable();
...
jQuery(document).ready(function() {
   ...
   $.fn.dataTable.ext.errMode = 'throw';
    ...
    table = $('#datahakabsensi').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_list')?>",
            "type": "POST",
             "timeout": 15000,
          ...
          $('td:eq(3),td:eq(4),td:eq(5)', nRow).addClass( "raport-mapel" );
    ...
    //$("input[name='checkAll']").click(function() {
                ...
                //$("#datahakabsensi tbody tr td div span input:checkbox").attr("checked", checked);
                //$("#datahakabsensi tbody tr td div span").attr("checked", checked);
            //});
            
 $('.group-checkable').change(function() {
                var set = $("#datahakabsensi tbody tr td div span input:checkbox");
                var checked = $(this).is(":checked");
                $(set).each(function() {
    ...
    // return  false;
    $.ajax({
        url: '<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_multiple_delete')?>',
        type: 'post',
...
function save()
{
   ...
   $('#kelas-cek2').val($('#data-select2-raport2 option:selected').text());
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_tambah')?>",
        type: "POST",
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
 
            $('[name="hakabsensi_id"]').val(data.hakabsensi_id);
            $('[name="hakabsensi_kodeguru"]').val(data.hakabsensi_kodeguru);
            $('[name="hakabsensi_kelas"]').val(data.hakabsensi_kelas);
            $('[name="hakabsensi_status"]').val(data.hakabsensi_status);

            $("#data-select2-raport3").val(data.hakabsensi_kodeguru).trigger("change");
             $("#data-select2-raport4").val(data.hakabsensi_kelas).trigger("change");
            
 
            $('#form2')[0].reset();

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Hak Absensi'); // Set title to Bootstrap modal title
...
{   
   ...
   $('#kelas-cek').val($('#data-select2-raport4 option:selected').text());
    // ajax update data to database
    $.ajax({
        url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_update')?>",
        type: "POST",
        data: $('#form').serialize(),
        dataType: "JSON",
        success: function(data)
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('4dm1n-D33H4RdY-n1c3dR34M/datahakabsensi/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
